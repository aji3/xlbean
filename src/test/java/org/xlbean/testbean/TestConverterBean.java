package org.xlbean.testbean;

import java.math.BigDecimal;
import java.math.BigInteger;
import java.time.LocalDate;
import java.time.LocalDateTime;
import java.time.LocalTime;
import java.util.Date;

public class TestConverterBean {

	private BigDecimal _BigDecimal;
	private BigInteger _BigInteger;
	private Boolean _Boolean;
	private Character _Character;
	private Date _Date;
	private Double _Double;
	private Integer _Integer;
	private LocalDate _LocalDate;
	private LocalDateTime _LocalDateTime;
	private LocalTime _LocalTime;
	private Long _Long;
	private Short _Short;
	private String _String;

	private boolean _boolean;
	private char _char;
	private double _double;
	private int _int;
	private long _long;
	private short _short;

	public BigDecimal get_BigDecimal() {
		return _BigDecimal;
	}

	public void set_BigDecimal(BigDecimal _BigDecimal) {
		this._BigDecimal = _BigDecimal;
	}

	public BigInteger get_BigInteger() {
		return _BigInteger;
	}

	public void set_BigInteger(BigInteger _BigInteger) {
		this._BigInteger = _BigInteger;
	}

	public Boolean get_Boolean() {
		return _Boolean;
	}

	public void set_Boolean(Boolean _Boolean) {
		this._Boolean = _Boolean;
	}

	public Character get_Character() {
		return _Character;
	}

	public void set_Character(Character _Character) {
		this._Character = _Character;
	}

	public Date get_Date() {
		return _Date;
	}

	public void set_Date(Date _Date) {
		this._Date = _Date;
	}

	public Double get_Double() {
		return _Double;
	}

	public void set_Double(Double _Double) {
		this._Double = _Double;
	}

	public Integer get_Integer() {
		return _Integer;
	}

	public void set_Integer(Integer _Integer) {
		this._Integer = _Integer;
	}

	public LocalDate get_LocalDate() {
		return _LocalDate;
	}

	public void set_LocalDate(LocalDate _LocalDate) {
		this._LocalDate = _LocalDate;
	}

	public LocalDateTime get_LocalDateTime() {
		return _LocalDateTime;
	}

	public void set_LocalDateTime(LocalDateTime _LocalDateTime) {
		this._LocalDateTime = _LocalDateTime;
	}

	public LocalTime get_LocalTime() {
		return _LocalTime;
	}

	public void set_LocalTime(LocalTime _LocalTime) {
		this._LocalTime = _LocalTime;
	}

	public Long get_Long() {
		return _Long;
	}

	public void set_Long(Long _Long) {
		this._Long = _Long;
	}

	public Short get_Short() {
		return _Short;
	}

	public void set_Short(Short _Short) {
		this._Short = _Short;
	}

	public String get_String() {
		return _String;
	}

	public void set_String(String _String) {
		this._String = _String;
	}

	public boolean is_boolean() {
		return _boolean;
	}

	public void set_boolean(boolean _boolean) {
		this._boolean = _boolean;
	}

	public char get_char() {
		return _char;
	}

	public void set_char(char _char) {
		this._char = _char;
	}

	public double get_double() {
		return _double;
	}

	public void set_double(double _double) {
		this._double = _double;
	}

	public int get_int() {
		return _int;
	}

	public void set_int(int _int) {
		this._int = _int;
	}

	public long get_long() {
		return _long;
	}

	public void set_long(long _long) {
		this._long = _long;
	}

	public short get_short() {
		return _short;
	}

	public void set_short(short _short) {
		this._short = _short;
	}

	@Override
	public String toString() {
		return "TestConverterBean [_BigDecimal=" + _BigDecimal + ", _BigInteger=" + _BigInteger + ", _Boolean="
				+ _Boolean + ", _Character=" + _Character + ", _Date=" + _Date + ", _Double=" + _Double + ", _Integer="
				+ _Integer + ", _LocalDate=" + _LocalDate + ", _LocalDateTime=" + _LocalDateTime + ", _LocalTime="
				+ _LocalTime + ", _Long=" + _Long + ", _Short=" + _Short + ", _String=" + _String + ", _boolean="
				+ _boolean + ", _char=" + _char + ", _double=" + _double + ", _int=" + _int + ", _long=" + _long
				+ ", _short=" + _short + "]";
	}

}
