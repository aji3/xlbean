...
public class BeanConverterImpl implements BeanConverter {
  ...
  /**
   * Convert the object retrieved by the given sourceKey to an object of given destinationClass.
   *
   * @param sourceKey
  ...
  private Map<Class<?>, Map<String, PropertyDescriptor>> classPropertyDescriptorMap =
      new HashMap<>();
  ...
  protected PropertyDescriptor getPropertyDescriptor(Class<?> clazz, String propertyName) {
    ...
    if (propertyDescriptorMap == null) {
      ...
      try {
        List<PropertyDescriptor> propertyList =
    ...
    for (Entry<String, Object> entry : sourceMap.entrySet()) {
      try {
        ...
        if (value instanceof Map) {
          ...
          List<Object> obj =
              (List<Object>)
                  instantiate(pd.getPropertyType().isInterface() ? ArrayList.class : type);
          ParameterizedType p =
              (ParameterizedType) pd.getWriteMethod().getGenericParameterTypes()[0];
          ...
          for (Object srcObj : (Iterable<?>) value) {
            if (isLeaf(srcObj.getClass())) {
              ...
              Object bean =
                  toBean((Map<String, Object>) srcObj, Class.forName(childType.getTypeName()));
              obj.add(bean);
            }
          }
          setter.invoke(destinationObj, obj);
          ...
          Object convertedValue =
              converter.toObject(value instanceof String ? (String) value : value.toString(), type);
