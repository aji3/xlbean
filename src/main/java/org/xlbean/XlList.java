...
/**
 * Sub class of {@link ArrayList} with additional utility methods suitable for common data structure
 * of excel.
 *
 * <p>All the tables defined in excel is represented as {@link XlList} of {@link XlBean} instances.
 *
...
public class XlList extends ArrayList<XlBean> {
  ...
  /**
   * Convert a denormalized list of one to many relation objects into parent object with list of
   * children objects.
   *
   * <p>Suppose you have a table of companies and employees in a excel sheet. In each row of the
   * table, there are a name of an employee and a name of a company which the employee belongs to.
   * When this table is loaded by XlBean, you will get an instance of {@link XlList} filled with
   * multiple instances of {@link XlBean} as elements, and each of the XlBean instance has name of
   * an employeeName and a companyName.
   *
   * <p>Now you want to have a list of companies which has list of employees inside each instance of
   * company. This can be done by calling this method with "employees" as childListName and
   * ["companyName"] as parentKeys.
   *
   * @param childListName
   * @param parentKeys
   * @return
   ...
   *
   * <p>Key of {@link XlBean} is a string created by concatenating string retrieved from beans
   * inside this list by {@code keys}. Delimiter for the key is "#_#".
   *
   * @param keys
   * @return
   */
   ...
   *
   * <p>All the keys and values in the {@code conditionMap} is used to check if elements in this
   * list matches with it.
   *
   * @param conditionMap
   * @return
   */
   ...
   *
   * <p>If the index is out of bounds, then it returns null.
   *
   * @param index
   * @return
   */
   ...
   * list matches with it.
   *
   * <p>The matching starts from index 0 to the end of this list one by one until matched element.
   * If there is no element to be matched, then it returns null.
   *
   * @param conditionMap
   * @return
   */
  ...
  /**
   * Returns an instance of {@link XlBean} which is registered to this list by an index which value
   * is specified by the {@code conditionMap}.
   *
   * <p>This method is available only when one index is set to table. Otherwise it always returns
   * null.
   *
   * @param conditionMap
   * @return
  ...
  /**
   * Set {@code data} at the specified {@code index}. If length of this list is less than specified,
   * then it will extend the list by filling with null.
   *
   * @param index
   * @param data
   */
