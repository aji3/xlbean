	...
	public List<SingleDefinition> getColumns() {
		if (columnsCache == null) {
			if (definition.isDirectionDown()) {
				columnsCache = definition.getAttributes().values().stream().filter(elem -> elem.getCell().getColumn() != null).collect(Collectors.toList());
			} else {
				columnsCache = definition.getAttributes().values().stream().filter(elem -> elem.getCell().getRow() != null).collect(Collectors.toList());
			}			
		}
		return columnsCache;
	}
