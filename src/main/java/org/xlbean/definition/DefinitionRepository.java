...
public class DefinitionRepository {
   ...
   *
   * <p>It compares values of {@link Definition#getDefinitionId()} of existing definition instances
   * with the given {@code definition}. If the same {@code definitionId} already exist, then it will
   * merge them. Otherwise, it will simply add the instance to this repository.
   *
   * @param definition
   */
  ...
  /**
   * Calls {@link Definition#validate()} of all the {@link Definition} instances in this repository,
   * then set the corresponding excel sheet instance to each definitions.
   *
   * @param workbook
   */
  public void validate(XlWorkbook workbook) {
    for (Definition definition : definitions) {
      ...
      if (!definition.validate()) {
        log.warn(
            "Invalid definition [{}] ({})", definition.getName(), definition.getClass().getName());
        continue;
      }
