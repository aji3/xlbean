...
public class ExcelR1C1DefinitionLoader extends DefinitionLoader<XlWorkbook> {
	...
	private static final List<DefinitionResolver> DEFAULT_DEFINITION_RESOLVERS = Arrays
			.asList(new SingleDefinitionResolver(), new TableDefinitionResolver());

    @Override
    public void initialize(Object definitionSource) {
        if (definitionSource == null || !(definitionSource instanceof Workbook)) {
            throw new IllegalArgumentException(String.format("Definition source should be an instance of %s", Workbook.class.getName()));
        }
	...
	public DefinitionRepository load() {
		...
		for (int i = 0; i < workbook.getNumberOfSheets(); i++) {
			...
			log.info("Loaded table definition from sheet {} [{} msec]", sheet.getSheetName(), (System.currentTimeMillis() - now));
		}
		return definitions;
	}
	...
	private void resolveDefinition(DefinitionRepository definitions, XlSheet sheet, int num, boolean isColumn) {
		String value = null;
		if (isColumn) {
		...
		List<String> splittedDefinitionStr = Arrays.stream(value.split(",")).map(str -> str.trim())
