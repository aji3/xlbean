 ...
 *
 * <p>To represent a structure of table, which is a set of pre-defined columns and any number of
 * rows, this class has a {@code attributes} property as a Map of {@link SingleDefinition}s. For a
 * column, name of the column and column number is set to {@link SingleDefinition} object and set to
 * {@code attributes}. For rows, only the start row number is set to {@link SingleDefinition} object
 * and set to {@code attributes}.
 *
...
public class TableDefinition extends Definition {
   ...
   *
   * <p>It returns false, which means direction is "down", when this definition meets either of the
  ...
  /**
   * Returns {@code attributes} which is a map to hold all the columns and start cell information.
   *
   * @return
   */
   ...
   *
   * <p>It iterates for all the {@code attributes} and merges them. If {@code name} of the given
   * instance and this instance is not the same, it writes out {@code warn} level log.
   *
   * @param newDefinition
   */
  @Override
