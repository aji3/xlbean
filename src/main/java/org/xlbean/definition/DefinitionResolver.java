...
public abstract class DefinitionResolver {
	...
	/**
	 * Parse key string and create a Definition object with cell information.
	 * Then register the Definition object to context and return.
	 * 
	 * @param context
	 * @param key
	 * @param cell
	...
	protected Map<String, String> parseOptions(String tableNameOrColumnName) {
		...
		if (tableNameOrColumnName.indexOf('?') > 0) {
			String optionString = tableNameOrColumnName.substring(tableNameOrColumnName.indexOf('?') + 1,
					tableNameOrColumnName.length());
			Arrays.stream(optionString.split("&")).map(str -> str.trim().split("="))
