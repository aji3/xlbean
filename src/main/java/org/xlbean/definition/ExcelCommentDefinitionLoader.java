...
public class ExcelCommentDefinitionLoader extends ExcelR1C1DefinitionLoader {
	...
	private static final List<DefinitionResolver> EXCEL_COMMENT_DEFINITION_RESOLVERS = Arrays
			.asList(new SingleDefinitionResolver(), 
	...
	private void resolveDefinition(DefinitionRepository definitions, XlSheet sheet, int row, int col) {
		...
		for (String definitionStr : splittedDefinitionStr) {
			...
			if (resolver == null) {
				...
				Definition newDefinition = resolver.resolve(definitionStr, new XlCellAddress.Builder().row(row).column(col).build());
