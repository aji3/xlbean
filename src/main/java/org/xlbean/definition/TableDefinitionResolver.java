...
public class TableDefinitionResolver extends DefinitionResolver {
    ...
    private static final String JAVA_LIST_VARIABLE_PATTERN = JAVA_VARIABLE_PATTERN + "(\\[[0-9]+\\])?";
    private static final String COMMA_CONNECTED_JAVA_VARIABLE_PATTERN = String.format(
        "(%s|%s\\.%s)(\\.%s|%s\\.%s)*",
        ...
        JAVA_LIST_VARIABLE_PATTERN,
        JAVA_VARIABLE_PATTERN);
    private static final String TABLE_OR_COLUMN_PATTERN = String.format(
        "%s(\\?%s=\\w*(&%s=\\w*)*)?",
        COMMA_CONNECTED_JAVA_VARIABLE_PATTERN,
        JAVA_VARIABLE_PATTERN,
        JAVA_VARIABLE_PATTERN);
    private static final Pattern RESOLVABLE_PATTERN = Pattern.compile(
        String.format(
            "%s#%s",
            TABLE_OR_COLUMN_PATTERN,
            TABLE_OR_COLUMN_PATTERN.replaceFirst("\\[a-z", "\\[~a-z")));
    ...
    public Definition resolve(String key, XlCellAddress cell) {
        String[] tableNameAndColumnNameArray = Arrays.stream(key.split("#")).map(str -> str.trim()).toArray(
