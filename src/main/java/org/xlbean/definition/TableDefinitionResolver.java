...
public class TableDefinitionResolver extends DefinitionResolver {
	...
	private static final String COMMA_CONNECTED_JAVA_VARIABLE_PATTERN = String.format("(%s|%s\\.%s)(\\.%s|%s\\.%s)*", JAVA_VARIABLE_PATTERN, JAVA_LIST_VARIABLE_PATTERN, JAVA_VARIABLE_PATTERN, JAVA_VARIABLE_PATTERN, JAVA_LIST_VARIABLE_PATTERN, JAVA_VARIABLE_PATTERN);
...
//	private static final Pattern RESOLVABLE_PATTERN = Pattern.compile("[a-zA-Z_]\\w*(\\.[a-zA-Z_]\\w*)*(\\?[a-zA-Z_]\\w*=\\w*(&[a-zA-Z_]\\w*=\\w*)*)?#[~a-zA-Z_]\\w*(\\.[a-zA-Z_]\\w*)*(\\?[a-zA-Z_]\\w*=\\w*(&[a-zA-Z_]\\w*=\\w*)*)?");
	...
	public Definition resolve(String key, XlCellAddress cell) {
		String[] tableNameAndColumnNameArray = Arrays.stream(key.split("#")).map(str -> str.trim()).toArray(size -> new String[size]);
