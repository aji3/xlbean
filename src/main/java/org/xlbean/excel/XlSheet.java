...
public class XlSheet {
  ...
  private static final Pattern ERROR_MESSAGE_PATTERN =
      Pattern.compile("Cannot get.*from a ERROR.*");
  ...
  /**
   * {@link Cell#getCellTypeEnum} is used because this method is to be replaced by existing method
  ...
  private boolean isDateTimeValue(String dataFormatString) {
    ...
    return dataFormatString
        .replaceAll("\\[.*\\]", "")
        .chars()
        .anyMatch(c -> DATETIMEFORMAT_CHARS.indexOf(c) >= 0);
  }
  ...
  /**
   * Format excel date value which is a differential number from 1900/1/0, to date-time
  ...
  protected String formatDateTimeValue(double value) {
    ...
    LocalDateTime dateTime =
        LocalDateTime.of(1899, 12, 30, 0, 0, 0, 0).plusDays((int) value).plusNanos(time);
  ...
  /**
   * Format excel date value which is a differential number from 1900/1/0, to time
  ...
  private void handleException(Exception e, Cell cell) {
    if (e instanceof FormulaParseException
      ...
      log.warn(
          "Excel error exists at (sheet:{}, row:{}, column:{})",
